...
function learndash_evidence_add_submenus() {
    ...
    // Add migration page for fixing dates
    add_submenu_page(
        'learndash-lms',
        ...
        'manage_options',
        'fix-course-dates',
        'learndash_evidence_fix_dates_page'
    );
}
...
// Migration page to fix course dates
function learndash_evidence_fix_dates_page() {
    if (isset($_POST['fix_dates']) && wp_verify_nonce($_POST['_wpnonce'], 'fix_course_dates')) {
        $fixed_count = learndash_evidence_fix_1970_dates();
    ...
    $affected_users = learndash_evidence_get_1970_users();
    if (!empty($affected_users)) {
        echo '<h3>' . esc_html__('Users with incorrect dates:', 'learndash-evidence') . '</h3>';
        echo '<table class="wp-list-table widefat striped">';
        echo '<thead><tr><th>Usuario</th><th>Fecha Registro</th><th>Cursos Afectados</th></tr></thead>';
        echo '<tbody>';
        foreach ($affected_users as $user_data) {
            ...
            echo '<td>' . implode(', ', array_map('get_the_title', $user_data['courses'])) . '</td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
...
// Get users with 1970 dates
function learndash_evidence_get_1970_users() {
    global $wpdb;

    // Get all course access meta keys with value 0 or timestamp equivalent to 1970-01-01
...
// Fix 1970 dates function
function learndash_evidence_fix_1970_dates() {
    $affected_users = learndash_evidence_get_1970_users();
    $fixed_count = 0;
...
// IMPROVED: Set course access date when user is created (covers registration)
function learndash_evidence_set_access_date_on_user_register($user_id) {
    $user = get_userdata($user_id);
    if (!$user) return;

    // Use registration date as the access date
    $registration_timestamp = strtotime($user->user_registered);
    ...
    foreach ($courses as $course_id) {
        ...
        // Set to registration date if empty, 0, or invalid
...
// IMPROVED: Set course access date when meta changes (covers manual assignment, imports, etc)
function learndash_evidence_set_access_date_on_meta_update($meta_id, $user_id, $meta_key, $meta_value) {
    if (preg_match('/^course_(\d+)_access_from$/', $meta_key, $matches)) {
        if (empty($meta_value) || $meta_value == '0' || $meta_value == 0 || $meta_value == 'false' || $meta_value <= 86400) {
            $user = get_userdata($user_id);
            if ($user) {
                $registration_timestamp = strtotime($user->user_registered);
                update_user_meta($user_id, $meta_key, $registration_timestamp);
...
// Hook into various LearnDash actions that might grant course access
add_action('ld_added_course_access', 'learndash_evidence_on_course_access_granted', 10, 2);
add_action('learndash_user_course_access_changed', 'learndash_evidence_on_course_access_granted', 10, 2);
